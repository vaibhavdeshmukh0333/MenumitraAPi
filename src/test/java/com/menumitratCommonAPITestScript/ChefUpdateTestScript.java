...
public class ChefUpdateTestScript extends APIBase {
    ...
    @DataProvider(name = "getChefUpdateNegativeData")
    public Object[][] getChefUpdateNegativeData() throws customException {
        try {
            LogUtils.info("Reading chef update negative test scenario data");
            ExtentReport.getTest().log(Status.INFO, "Reading chef update negative test scenario data");
            ...
            if (filteredData.isEmpty()) {
                String errorMsg = "No valid chef update negative test data found after filtering";
                LogUtils.failure(logger, errorMsg);
                ExtentReport.getTest().log(Status.FAIL, MarkupHelper.createLabel(errorMsg, ExtentColor.RED));
                throw new customException(errorMsg);
            }
            ...
            LogUtils.failure(logger, "Error in getting chef update negative test data: " + e.getMessage());
            ExtentReport.getTest().log(Status.FAIL, "Error in getting chef update negative test data: " + e.getMessage());
            throw new customException("Error in getting chef update negative test data: " + e.getMessage());
        }
    }

    @Test(dataProvider = "getChefUpdateNegativeData")
    public void chefUpdateNegativeTest(String apiName, String testCaseid, String testType, String description,
            String httpsmethod, String requestBody, String expectedResponseBody, String statusCode) throws customException {
        try {
            LogUtils.info("Starting chef update negative test case: " + testCaseid);
            ExtentReport.createTest("Chef Update Negative Test - " + testCaseid + ": " + description);
            ExtentReport.getTest().log(Status.INFO, "Test Description: " + description);
            
            if (apiName.equalsIgnoreCase("chefupdate") && testType.equalsIgnoreCase("negative")) {
                requestBodyJson = new JSONObject(requestBody);
                
                LogUtils.info("Request Body: " + requestBodyJson.toString());
                ExtentReport.getTest().log(Status.INFO, "Request Body: " + requestBodyJson.toString());
                
                // Set payload for chef update request
                ...
                // Validate status code
                    ...
                    LogUtils.success(logger, "Chef update negative test completed successfully");
                    ExtentReport.getTest().log(Status.PASS, MarkupHelper.createLabel("Chef update negative test completed successfully", ExtentColor.GREEN));
                }
            ...
            String errorMsg = "Error in chef update negative test: " + e.getMessage();
