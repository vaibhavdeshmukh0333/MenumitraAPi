...
public class CheckVersionTestScript extends APIBase
{
    ...
    @DataProvider(name = "getCheckVersionNegativeData")
    public Object[][] getCheckVersionNegativeData() throws customException {
        try {
            LogUtils.info("Reading check version negative test scenario data");
            ExtentReport.getTest().log(Status.INFO, "Reading check version negative test scenario data");
            ...
            if (filteredData.isEmpty()) {
                String errorMsg = "No valid check version negative test data found after filtering";
                LogUtils.failure(logger, errorMsg);
                ExtentReport.getTest().log(Status.FAIL, MarkupHelper.createLabel(errorMsg, ExtentColor.RED));
                throw new customException(errorMsg);
            }
            ...
            LogUtils.failure(logger, "Error in getting check version negative test data: " + e.getMessage());
            ExtentReport.getTest().log(Status.FAIL, "Error in getting check version negative test data: " + e.getMessage());
            throw new customException("Error in getting check version negative test data: " + e.getMessage());
        }
    }

    @Test(dataProvider = "getCheckVersionNegativeData")
    public void checkVersionNegativeTest(String apiName, String testCaseid, String testType, String description,
            String httpsmethod, String requestBody, String expectedResponseBody, String statusCode) throws customException {
        try {
            LogUtils.info("Starting check version negative test case: " + testCaseid);
            ExtentReport.createTest("Check Version Negative Test - " + testCaseid + ": " + description);
            ExtentReport.getTest().log(Status.INFO, "Test Description: " + description);
            
            if (apiName.equalsIgnoreCase("checkversion") && testType.equalsIgnoreCase("negative")) {
                requestBodyJson = new JSONObject(requestBody);
                
                LogUtils.info("Request Body: " + requestBodyJson.toString());
                ExtentReport.getTest().log(Status.INFO, "Request Body: " + requestBodyJson.toString());
                
                // Set payload for check version request
                if (requestBodyJson.has("app_type")) {
                    checkVersionRequest.setApp_type(requestBodyJson.getString("app_type"));
                }
                ...
                // Validate status code
                    ...
                    LogUtils.success(logger, "Check version negative test completed successfully");
                    ExtentReport.getTest().log(Status.PASS, MarkupHelper.createLabel("Check version negative test completed successfully", ExtentColor.GREEN));
                }
            ...
            String errorMsg = "Error in check version negative test: " + e.getMessage();
            LogUtils.exception(logger, errorMsg, e);
            ExtentReport.getTest().log(Status.FAIL, MarkupHelper.createLabel(errorMsg, ExtentColor.RED));
            if (response != null) {
                ExtentReport.getTest().log(Status.FAIL, "Failed Response Status Code: " + response.getStatusCode());
