...
public class RemoveTemplateTestScript extends APIBase
{
    ...
    @DataProvider(name = "getRemoveTemplateNegativeData")
    public Object[][] getRemoveTemplateNegativeData() throws customException {
        try {
            LogUtils.info("Reading remove template negative test scenario data");
            ExtentReport.getTest().log(Status.INFO, "Reading remove template negative test scenario data");
            ...
            if (filteredData.isEmpty()) {
                String errorMsg = "No valid remove template negative test data found after filtering";
                LogUtils.failure(logger, errorMsg);
                ExtentReport.getTest().log(Status.FAIL, MarkupHelper.createLabel(errorMsg, ExtentColor.RED));
                throw new customException(errorMsg);
            }
            ...
            LogUtils.failure(logger, "Error in getting remove template negative test data: " + e.getMessage());
            ExtentReport.getTest().log(Status.FAIL, "Error in getting remove template negative test data: " + e.getMessage());
            throw new customException("Error in getting remove template negative test data: " + e.getMessage());
        }
    }

    @Test(dataProvider = "getRemoveTemplateNegativeData")
    public void removeTemplateNegativeTest(String apiName, String testCaseid, String testType, String description,
            String httpsmethod, String requestBody, String expectedResponseBody, String statusCode) throws customException {
        try {
            LogUtils.info("Starting remove template negative test case: " + testCaseid);
            ExtentReport.createTest("Remove Template Negative Test - " + testCaseid + ": " + description);
            ExtentReport.getTest().log(Status.INFO, "Test Description: " + description);
            
            if (apiName.equalsIgnoreCase("removetemplate") && testType.equalsIgnoreCase("negative")) {
                requestBodyJson = new JSONObject(requestBody);
                
                LogUtils.info("Request Body: " + requestBodyJson.toString());
                ExtentReport.getTest().log(Status.INFO, "Request Body: " + requestBodyJson.toString());
                
                // Set payload for remove template request
                ...
                // Validate status code
                    ...
                    LogUtils.success(logger, "Remove template negative test completed successfully");
                    ExtentReport.getTest().log(Status.PASS, MarkupHelper.createLabel("Remove template negative test completed successfully", ExtentColor.GREEN));
                }
            ...
            String errorMsg = "Error in remove template negative test: " + e.getMessage();
