...
public class WaiterListViewTestScript extends APIBase
{
    ...
    @DataProvider(name = "getWaiterListViewNegativeData")
    public Object[][] getWaiterListViewNegativeData() throws customException {
        try {
            LogUtils.info("Reading waiter list view negative test scenario data");
            ExtentReport.getTest().log(Status.INFO, "Reading waiter list view negative test scenario data");
            ...
            if (filteredData.isEmpty()) {
                String errorMsg = "No valid waiter list view negative test data found after filtering";
                LogUtils.failure(logger, errorMsg);
                ExtentReport.getTest().log(Status.FAIL, MarkupHelper.createLabel(errorMsg, ExtentColor.RED));
                throw new customException(errorMsg);
            }
            ...
            LogUtils.failure(logger, "Error in getting waiter list view negative test data: " + e.getMessage());
            ExtentReport.getTest().log(Status.FAIL, "Error in getting waiter list view negative test data: " + e.getMessage());
            throw new customException("Error in getting waiter list view negative test data: " + e.getMessage());
        }
    }
    
    @Test(dataProvider = "getWaiterListViewNegativeData")
    public void waiterListViewNegativeTest(String apiName, String testCaseid, String testType, String description,
            String httpsmethod, String requestBody, String expectedResponseBody, String statusCode) throws customException {
        try {
            LogUtils.info("Starting waiter list view negative test case: " + testCaseid);
            ExtentReport.createTest("Waiter List View Negative Test - " + testCaseid + ": " + description);
            ExtentReport.getTest().log(Status.INFO, "Test Description: " + description);
            
            if (apiName.equalsIgnoreCase("waiterlistview") && testType.equalsIgnoreCase("negative")) {
                requestBodyJson = new JSONObject(requestBody);
                
                LogUtils.info("Request Body: " + requestBodyJson.toString());
                ExtentReport.getTest().log(Status.INFO, "Request Body: " + requestBodyJson.toString());
                
                // Set payload for waiter list view request
                ...
                // Validate status code
                    ...
                    LogUtils.success(logger, "Waiter list view negative test completed successfully");
                    ExtentReport.getTest().log(Status.PASS, MarkupHelper.createLabel("Waiter list view negative test completed successfully", ExtentColor.GREEN));
                }
            ...
            String errorMsg = "Error in waiter list view negative test: " + e.getMessage();
            LogUtils.exception(logger, errorMsg, e);
            ExtentReport.getTest().log(Status.FAIL, MarkupHelper.createLabel(errorMsg, ExtentColor.RED));
            if (response != null) {
                ExtentReport.getTest().log(Status.FAIL, "Failed Response Status Code: " + response.getStatusCode());
