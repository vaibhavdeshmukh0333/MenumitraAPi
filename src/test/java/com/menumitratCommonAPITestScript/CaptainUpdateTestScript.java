...
public class CaptainUpdateTestScript extends APIBase
{
    ...
    @DataProvider(name = "getCaptainUpdateNegativeData")
    public Object[][] getCaptainUpdateNegativeData() throws customException {
        try {
            LogUtils.info("Reading captain update negative test scenario data");
            ExtentReport.getTest().log(Status.INFO, "Reading captain update negative test scenario data");
            ...
            if (filteredData.isEmpty()) {
                String errorMsg = "No valid captain update negative test data found after filtering";
                LogUtils.failure(logger, errorMsg);
                ExtentReport.getTest().log(Status.FAIL, MarkupHelper.createLabel(errorMsg, ExtentColor.RED));
                throw new customException(errorMsg);
            }
            ...
            LogUtils.failure(logger, "Error in getting captain update negative test data: " + e.getMessage());
            ExtentReport.getTest().log(Status.FAIL, "Error in getting captain update negative test data: " + e.getMessage());
            throw new customException("Error in getting captain update negative test data: " + e.getMessage());
        }
    }

    @Test(dataProvider = "getCaptainUpdateNegativeData")
    public void captainUpdateNegativeTest(String apiName, String testCaseid, String testType, String description,
            String httpsmethod, String requestBody, String expectedResponseBody, String statusCode) throws customException {
        try {
            LogUtils.info("Starting captain update negative test case: " + testCaseid);
            ExtentReport.createTest("Captain Update Negative Test - " + testCaseid + ": " + description);
            ExtentReport.getTest().log(Status.INFO, "Test Description: " + description);
            
            if (apiName.equalsIgnoreCase("captainupdate") && testType.equalsIgnoreCase("negative")) {
                requestBodyJson = new JSONObject(requestBody);
                
                LogUtils.info("Request Body: " + requestBodyJson.toString());
                ExtentReport.getTest().log(Status.INFO, "Request Body: " + requestBodyJson.toString());
                
                // Set payload for captain update request
                ...
                // Validate status code
                    ...
                    LogUtils.success(logger, "Captain update negative test completed successfully");
                    ExtentReport.getTest().log(Status.PASS, MarkupHelper.createLabel("Captain update negative test completed successfully", ExtentColor.GREEN));
                }
            ...
            String errorMsg = "Error in captain update negative test: " + e.getMessage();
            LogUtils.exception(logger, errorMsg, e);
            ExtentReport.getTest().log(Status.FAIL, MarkupHelper.createLabel(errorMsg, ExtentColor.RED));
            if (response != null) {
                ExtentReport.getTest().log(Status.FAIL, "Failed Response Status Code: " + response.getStatusCode());
