...
public class WaiterDeleteTestScript extends APIBase
{
    ...
    @DataProvider(name = "getWaiterDeleteNegativeData")
    public Object[][] getWaiterDeleteNegativeData() throws customException {
        try {
            LogUtils.info("Reading waiter delete negative test scenario data");
            ExtentReport.getTest().log(Status.INFO, "Reading waiter delete negative test scenario data");
            ...
            if (filteredData.isEmpty()) {
                String errorMsg = "No valid waiter delete negative test data found after filtering";
                LogUtils.failure(logger, errorMsg);
                ExtentReport.getTest().log(Status.FAIL, MarkupHelper.createLabel(errorMsg, ExtentColor.RED));
                throw new customException(errorMsg);
            }
            ...
            LogUtils.failure(logger, "Error in getting waiter delete negative test data: " + e.getMessage());
            ExtentReport.getTest().log(Status.FAIL, "Error in getting waiter delete negative test data: " + e.getMessage());
            throw new customException("Error in getting waiter delete negative test data: " + e.getMessage());
        }
    }
    ...
    public void waiterDeleteNegativeTest(String apiName, String testCaseid, String testType, String description,
        ...
        try {
            LogUtils.info("Starting waiter delete negative test case: " + testCaseid);
            ExtentReport.createTest("Waiter Delete Negative Test - " + testCaseid + ": " + description);
            ExtentReport.getTest().log(Status.INFO, "Test Description: " + description);
            
            if (apiName.equalsIgnoreCase("waiterdelete") && testType.equalsIgnoreCase("negative")) {
                requestBodyJson = new JSONObject(requestBody);
                
                LogUtils.info("Request Body: " + requestBodyJson.toString());
                ExtentReport.getTest().log(Status.INFO, "Request Body: " + requestBodyJson.toString());
                
                // Set payload for waiter delete request
                ...
                waiterDeleteRequest.setupdate_user_id(String.valueOf(user_id));
                ...
                // Validate status code
                    ...
                    LogUtils.success(logger, "Waiter delete negative test completed successfully");
                    ExtentReport.getTest().log(Status.PASS, MarkupHelper.createLabel("Waiter delete negative test completed successfully", ExtentColor.GREEN));
                }
            ...
            String errorMsg = "Error in waiter delete negative test: " + e.getMessage();
