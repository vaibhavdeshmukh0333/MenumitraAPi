...
public class GetAvailableTablesTestScript extends APIBase
{
    ...
    @DataProvider(name = "getAvailableTablesNegativeData")
    public Object[][] getAvailableTablesNegativeData() throws customException {
        try {
            LogUtils.info("Reading available tables negative test scenario data");
            ExtentReport.getTest().log(Status.INFO, "Reading available tables negative test scenario data");
            ...
            if (filteredData.isEmpty()) {
                String errorMsg = "No valid available tables negative test data found after filtering";
                LogUtils.failure(logger, errorMsg);
                ExtentReport.getTest().log(Status.FAIL, MarkupHelper.createLabel(errorMsg, ExtentColor.RED));
                throw new customException(errorMsg);
            }
            ...
            LogUtils.failure(logger, "Error in getting available tables negative test data: " + e.getMessage());
            ExtentReport.getTest().log(Status.FAIL, "Error in getting available tables negative test data: " + e.getMessage());
            throw new customException("Error in getting available tables negative test data: " + e.getMessage());
        }
    }

    @Test(dataProvider = "getAvailableTablesNegativeData")
    public void getAvailableTablesNegativeTest(String apiName, String testCaseid, String testType, String description,
            String httpsmethod, String requestBody, String expectedResponseBody, String statusCode) throws customException {
        try {
            LogUtils.info("Starting get available tables negative test case: " + testCaseid);
            ExtentReport.createTest("Get Available Tables Negative Test - " + testCaseid + ": " + description);
            ExtentReport.getTest().log(Status.INFO, "Test Description: " + description);
            
            if (apiName.equalsIgnoreCase("getavailabletables") && testType.equalsIgnoreCase("negative")) {
                requestBodyJson = new JSONObject(requestBody);
                
                LogUtils.info("Request Body: " + requestBodyJson.toString());
                ExtentReport.getTest().log(Status.INFO, "Request Body: " + requestBodyJson.toString());
                
                // Set payload for available tables request
                ...
                // Validate status code
                    ...
                    LogUtils.success(logger, "Get available tables negative test completed successfully");
                    ExtentReport.getTest().log(Status.PASS, MarkupHelper.createLabel("Get available tables negative test completed successfully", ExtentColor.GREEN));
                }
            ...
            String errorMsg = "Error in get available tables negative test: " + e.getMessage();
            LogUtils.exception(logger, errorMsg, e);
            ExtentReport.getTest().log(Status.FAIL, MarkupHelper.createLabel(errorMsg, ExtentColor.RED));
            if (response != null) {
                ExtentReport.getTest().log(Status.FAIL, "Failed Response Status Code: " + response.getStatusCode());
