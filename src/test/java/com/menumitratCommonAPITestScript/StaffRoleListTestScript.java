...
public class StaffRoleListTestScript extends APIBase
{
    ...
    @DataProvider(name = "getStaffRoleListNegativeData")
    public Object[][] getStaffRoleListNegativeData() throws customException {
        try {
            LogUtils.info("Reading staff role list negative test scenario data");
            ExtentReport.getTest().log(Status.INFO, "Reading staff role list negative test scenario data");
            ...
            if (filteredData.isEmpty()) {
                String errorMsg = "No valid staff role list negative test data found after filtering";
                LogUtils.failure(logger, errorMsg);
                ExtentReport.getTest().log(Status.FAIL, MarkupHelper.createLabel(errorMsg, ExtentColor.RED));
                throw new customException(errorMsg);
            }
            ...
            LogUtils.failure(logger, "Error in getting staff role list negative test data: " + e.getMessage());
            ExtentReport.getTest().log(Status.FAIL, "Error in getting staff role list negative test data: " + e.getMessage());
            throw new customException("Error in getting staff role list negative test data: " + e.getMessage());
        }
    }

    @Test(dataProvider = "getStaffRoleListNegativeData")
    public void staffRoleListNegativeTest(String apiName, String testCaseid, String testType, String description,
            String httpsmethod, String requestBody, String expectedResponseBody, String statusCode) throws customException {
        try {
            LogUtils.info("Starting staff role list negative test case: " + testCaseid);
            ExtentReport.createTest("Staff Role List Negative Test - " + testCaseid + ": " + description);
            ExtentReport.getTest().log(Status.INFO, "Test Description: " + description);
            
            if (apiName.equalsIgnoreCase("staffrolelist") && testType.equalsIgnoreCase("negative")) {
                requestBodyJson = new JSONObject(requestBody);
                
                LogUtils.info("Request Body: " + requestBodyJson.toString());
                ExtentReport.getTest().log(Status.INFO, "Request Body: " + requestBodyJson.toString());
                
                // Set payload for staff role list request
                ...
                // Validate status code
                    ...
                    LogUtils.success(logger, "Staff role list negative test completed successfully");
                    ExtentReport.getTest().log(Status.PASS, MarkupHelper.createLabel("Staff role list negative test completed successfully", ExtentColor.GREEN));
                }
            ...
            String errorMsg = "Error in staff role list negative test: " + e.getMessage();
            LogUtils.exception(logger, errorMsg, e);
            ExtentReport.getTest().log(Status.FAIL, MarkupHelper.createLabel(errorMsg, ExtentColor.RED));
            if (response != null) {
                ExtentReport.getTest().log(Status.FAIL, "Failed Response Status Code: " + response.getStatusCode());
