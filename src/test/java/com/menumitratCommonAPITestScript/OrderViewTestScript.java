...
public class OrderViewTestScript extends APIBase
{
    ...
    @DataProvider(name = "getOrderViewNegativeData")
    public Object[][] getOrderViewNegativeData() throws customException {
        try {
            LogUtils.info("Reading order view negative test scenario data");
            ExtentReport.getTest().log(Status.INFO, "Reading order view negative test scenario data");
            ...
            if (filteredData.isEmpty()) {
                String errorMsg = "No valid order view negative test data found after filtering";
                LogUtils.failure(logger, errorMsg);
                ExtentReport.getTest().log(Status.FAIL, MarkupHelper.createLabel(errorMsg, ExtentColor.RED));
                throw new customException(errorMsg);
            }
            ...
            LogUtils.failure(logger, "Error in getting order view negative test data: " + e.getMessage());
            ExtentReport.getTest().log(Status.FAIL, "Error in getting order view negative test data: " + e.getMessage());
            throw new customException("Error in getting order view negative test data: " + e.getMessage());
        }
    }

    @Test(dataProvider = "getOrderViewNegativeData")
    public void orderViewNegativeTest(String apiName, String testCaseid, String testType, String description,
            String httpsmethod, String requestBody, String expectedResponseBody, String statusCode) throws customException {
        try {
            LogUtils.info("Starting order view negative test case: " + testCaseid);
            ExtentReport.createTest("Order View Negative Test - " + testCaseid + ": " + description);
            ExtentReport.getTest().log(Status.INFO, "Test Description: " + description);
            
            if (apiName.equalsIgnoreCase("orderview") && testType.equalsIgnoreCase("negative")) {
                requestBodyJson = new JSONObject(requestBody);
                
                LogUtils.info("Request Body: " + requestBodyJson.toString());
                ExtentReport.getTest().log(Status.INFO, "Request Body: " + requestBodyJson.toString());
                
                // Set payload for order view request
                ...
                if (requestBodyJson.has("order_number")) {
                    orderViewRequest.setOrder_number(requestBodyJson.getString("order_number"));
                }
                ...
                // Validate status code
                    ...
                    LogUtils.success(logger, "Order view negative test completed successfully");
                    ExtentReport.getTest().log(Status.PASS, MarkupHelper.createLabel("Order view negative test completed successfully", ExtentColor.GREEN));
                }
            ...
            String errorMsg = "Error in order view negative test: " + e.getMessage();
            LogUtils.exception(logger, errorMsg, e);
            ExtentReport.getTest().log(Status.FAIL, MarkupHelper.createLabel(errorMsg, ExtentColor.RED));
            if (response != null) {
                ExtentReport.getTest().log(Status.FAIL, "Failed Response Status Code: " + response.getStatusCode());
