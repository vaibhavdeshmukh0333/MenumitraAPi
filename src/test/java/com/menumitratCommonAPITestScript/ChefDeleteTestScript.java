...
public class ChefDeleteTestScript extends APIBase
{
    ...
    @DataProvider(name = "getChefDeleteNegativeData")
    public Object[][] getChefDeleteNegativeData() throws customException {
        try {
            LogUtils.info("Reading chef delete negative test scenario data");
            ExtentReport.getTest().log(Status.INFO, "Reading chef delete negative test scenario data");
            ...
            if (filteredData.isEmpty()) {
                String errorMsg = "No valid chef delete negative test data found after filtering";
                LogUtils.failure(logger, errorMsg);
                ExtentReport.getTest().log(Status.FAIL, MarkupHelper.createLabel(errorMsg, ExtentColor.RED));
                throw new customException(errorMsg);
            }
            ...
            LogUtils.failure(logger, "Error in getting chef delete negative test data: " + e.getMessage());
            ExtentReport.getTest().log(Status.FAIL, "Error in getting chef delete negative test data: " + e.getMessage());
            throw new customException("Error in getting chef delete negative test data: " + e.getMessage());
        }
    }

    @Test(dataProvider = "getChefDeleteNegativeData")
    public void chefDeleteNegativeTest(String apiName, String testCaseid, String testType, String description,
            String httpsmethod, String requestBody, String expectedResponseBody, String statusCode) throws customException {
        try {
            LogUtils.info("Starting chef delete negative test case: " + testCaseid);
            ExtentReport.createTest("Chef Delete Negative Test - " + testCaseid + ": " + description);
            ExtentReport.getTest().log(Status.INFO, "Test Description: " + description);
            
            if (apiName.equalsIgnoreCase("chefdelete") && testType.equalsIgnoreCase("negative")) {
                requestBodyJson = new JSONObject(requestBody);
                
                LogUtils.info("Request Body: " + requestBodyJson.toString());
                ExtentReport.getTest().log(Status.INFO, "Request Body: " + requestBodyJson.toString());
                
                // Set payload for chef delete request
                if (requestBodyJson.has("outlet_id")) {
                    chefDeleteRequest.setOutlet_id(String.valueOf(requestBodyJson.getString("outlet_id")));
                }
                if (requestBodyJson.has("user_id")) {
                    chefDeleteRequest.setUser_id(String.valueOf(requestBodyJson.getString("user_id")));
                }
                chefDeleteRequest.setUpdate_user_id(String.valueOf(user_id));
                ...
                // Validate status code
                    ...
                    LogUtils.success(logger, "Chef delete negative test completed successfully");
                    ExtentReport.getTest().log(Status.PASS, MarkupHelper.createLabel("Chef delete negative test completed successfully", ExtentColor.GREEN));
                }
            ...
            String errorMsg = "Error in chef delete negative test: " + e.getMessage();
