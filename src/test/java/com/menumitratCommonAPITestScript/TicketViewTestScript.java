...
public class TicketViewTestScript extends APIBase
{
    ...
    @DataProvider(name = "getTicketViewNegativeData")
    public Object[][] getTicketViewNegativeData() throws customException {
        try {
            LogUtils.info("Reading ticket view negative test scenario data");
            ExtentReport.getTest().log(Status.INFO, "Reading ticket view negative test scenario data");
            ...
            if (filteredData.isEmpty()) {
                String errorMsg = "No valid ticket view negative test data found after filtering";
                LogUtils.failure(logger, errorMsg);
                ExtentReport.getTest().log(Status.FAIL, MarkupHelper.createLabel(errorMsg, ExtentColor.RED));
                throw new customException(errorMsg);
            }
            ...
            LogUtils.failure(logger, "Error in getting ticket view negative test data: " + e.getMessage());
            ExtentReport.getTest().log(Status.FAIL, "Error in getting ticket view negative test data: " + e.getMessage());
            throw new customException("Error in getting ticket view negative test data: " + e.getMessage());
        }
    }

    @Test(dataProvider = "getTicketViewNegativeData")
    public void ticketViewNegativeTest(String apiName, String testCaseid, String testType, String description,
            String httpsmethod, String requestBody, String expectedResponseBody, String statusCode) throws customException {
        try {
            LogUtils.info("Starting ticket view negative test case: " + testCaseid);
            ExtentReport.createTest("Ticket View Negative Test - " + testCaseid + ": " + description);
            ExtentReport.getTest().log(Status.INFO, "Test Description: " + description);
            
            if (apiName.equalsIgnoreCase("ticketview") && testType.equalsIgnoreCase("negative")) {
                requestBodyJson = new JSONObject(requestBody);
                
                LogUtils.info("Request Body: " + requestBodyJson.toString());
                ExtentReport.getTest().log(Status.INFO, "Request Body: " + requestBodyJson.toString());
                
                // Set payload for ticket view request
                if (requestBodyJson.has("ticket_id")) {
                    ticketViewRequest.setTicket_id(requestBodyJson.getString("ticket_id"));
                }
                ...
                // Validate status code
                    ...
                    LogUtils.success(logger, "Ticket view negative test completed successfully");
                    ExtentReport.getTest().log(Status.PASS, MarkupHelper.createLabel("Ticket view negative test completed successfully", ExtentColor.GREEN));
                }
            ...
            String errorMsg = "Error in ticket view negative test: " + e.getMessage();
            LogUtils.exception(logger, errorMsg, e);
            ExtentReport.getTest().log(Status.FAIL, MarkupHelper.createLabel(errorMsg, ExtentColor.RED));
            if (response != null) {
                ExtentReport.getTest().log(Status.FAIL, "Failed Response Status Code: " + response.getStatusCode());
