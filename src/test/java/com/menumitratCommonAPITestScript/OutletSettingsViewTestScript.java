...
public class OutletSettingsViewTestScript extends APIBase
{
    ...
    private void outletSettingsView(String apiName, String testCaseid, String testType, String description,
        ...
        try {
            ...
            if (response.getStatusCode() == Integer.parseInt(statusCode)) {
                ...
                ExtentReport.getTest().log(Status.PASS, MarkupHelper.createLabel("Outlet settings view completed successfully", ExtentColor.GREEN));
                LogUtils.success(logger, "Outlet settings view completed successfully");
            } else {
    ...
    @DataProvider(name = "getOutletSettingsViewNegativeData")
    public Object[][] getOutletSettingsViewNegativeData() throws customException {
        try {
            LogUtils.info("Reading outlet settings view negative test scenario data");
            ExtentReport.getTest().log(Status.INFO, "Reading outlet settings view negative test scenario data");
            ...
            if (filteredData.isEmpty()) {
                String errorMsg = "No valid outlet settings view negative test data found after filtering";
                LogUtils.failure(logger, errorMsg);
                ExtentReport.getTest().log(Status.FAIL, MarkupHelper.createLabel(errorMsg, ExtentColor.RED));
                throw new customException(errorMsg);
            }
            ...
            LogUtils.failure(logger, "Error in getting outlet settings view negative test data: " + e.getMessage());
            ExtentReport.getTest().log(Status.FAIL, "Error in getting outlet settings view negative test data: " + e.getMessage());
            throw new customException("Error in getting outlet settings view negative test data: " + e.getMessage());
        }
    }

    @Test(dataProvider = "getOutletSettingsViewNegativeData")
    public void outletSettingsViewNegativeTest(String apiName, String testCaseid, String testType, String description,
            String httpsmethod, String requestBody, String expectedResponseBody, String statusCode) throws customException {
        try {
            LogUtils.info("Starting outlet settings view negative test case: " + testCaseid);
            ExtentReport.createTest("Outlet Settings View Negative Test - " + testCaseid + ": " + description);
            ExtentReport.getTest().log(Status.INFO, "Test Description: " + description);
            
            if (apiName.equalsIgnoreCase("outletsettingsview") && testType.equalsIgnoreCase("negative")) {
                requestBodyJson = new JSONObject(requestBody);
                
                LogUtils.info("Request Body: " + requestBodyJson.toString());
                ExtentReport.getTest().log(Status.INFO, "Request Body: " + requestBodyJson.toString());
                
                // Set payload for outlet settings view request
                ...
                // Validate status code
                    ...
                    LogUtils.success(logger, "Outlet settings view negative test completed successfully");
                    ExtentReport.getTest().log(Status.PASS, MarkupHelper.createLabel("Outlet settings view negative test completed successfully", ExtentColor.GREEN));
                }
            ...
            String errorMsg = "Error in outlet settings view negative test: " + e.getMessage();
