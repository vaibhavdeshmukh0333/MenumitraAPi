...
public class GetStaffListWithRole extends APIBase
{
    ...
    @DataProvider(name = "getStaffListWithRoleNegativeData")
    public Object[][] getStaffListWithRoleNegativeData() throws customException {
        try {
            LogUtils.info("Reading staff list with role negative test scenario data");
            ExtentReport.getTest().log(Status.INFO, "Reading staff list with role negative test scenario data");
            ...
            if (filteredData.isEmpty()) {
                String errorMsg = "No valid staff list with role negative test data found after filtering";
                LogUtils.failure(logger, errorMsg);
                ExtentReport.getTest().log(Status.FAIL, MarkupHelper.createLabel(errorMsg, ExtentColor.RED));
                throw new customException(errorMsg);
            }
            ...
            LogUtils.failure(logger, "Error in getting staff list with role negative test data: " + e.getMessage());
            ExtentReport.getTest().log(Status.FAIL, "Error in getting staff list with role negative test data: " + e.getMessage());
            throw new customException("Error in getting staff list with role negative test data: " + e.getMessage());
        }
    }

    @Test(dataProvider = "getStaffListWithRoleNegativeData")
    public void staffListWithRoleNegativeTest(String apiName, String testCaseid, String testType, String description,
            String httpsmethod, String requestBody, String expectedResponseBody, String statusCode) throws customException {
        try {
            LogUtils.info("Starting staff list with role negative test case: " + testCaseid);
            ExtentReport.createTest("Staff List With Role Negative Test - " + testCaseid + ": " + description);
            ExtentReport.getTest().log(Status.INFO, "Test Description: " + description);
            
            if (apiName.equalsIgnoreCase("getstafflistwithrole") && testType.equalsIgnoreCase("negative")) {
                requestBodyJson = new JSONObject(requestBody);
                
                LogUtils.info("Request Body: " + requestBodyJson.toString());
                ExtentReport.getTest().log(Status.INFO, "Request Body: " + requestBodyJson.toString());
                
                // Set payload for staff list with role request
                if (requestBodyJson.has("outlet_id")) {
                    staffListWithRoleRequest.setOutlet_id(Integer.parseInt(requestBodyJson.getString("outlet_id")));
                }
                ...
                // Validate status code
                    ...
                    LogUtils.success(logger, "Staff list with role negative test completed successfully");
                    ExtentReport.getTest().log(Status.PASS, MarkupHelper.createLabel("Staff list with role negative test completed successfully", ExtentColor.GREEN));
                }
            ...
            String errorMsg = "Error in staff list with role negative test: " + e.getMessage();
            LogUtils.exception(logger, errorMsg, e);
            ExtentReport.getTest().log(Status.FAIL, MarkupHelper.createLabel(errorMsg, ExtentColor.RED));
            if (response != null) {
                ExtentReport.getTest().log(Status.FAIL, "Failed Response Status Code: " + response.getStatusCode());
