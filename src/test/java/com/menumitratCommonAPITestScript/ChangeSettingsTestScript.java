...
public class ChangeSettingsTestScript extends APIBase
{
    ...
    @DataProvider(name = "getChangeSettingsNegativeData")
    public Object[][] getChangeSettingsNegativeData() throws customException {
        try {
            LogUtils.info("Reading change settings negative test scenario data");
            ExtentReport.getTest().log(Status.INFO, "Reading change settings negative test scenario data");
            ...
            if (filteredData.isEmpty()) {
                String errorMsg = "No valid change settings negative test data found after filtering";
                LogUtils.failure(logger, errorMsg);
                ExtentReport.getTest().log(Status.FAIL, MarkupHelper.createLabel(errorMsg, ExtentColor.RED));
                throw new customException(errorMsg);
            }
            ...
            LogUtils.failure(logger, "Error in getting change settings negative test data: " + e.getMessage());
            ExtentReport.getTest().log(Status.FAIL, "Error in getting change settings negative test data: " + e.getMessage());
            throw new customException("Error in getting change settings negative test data: " + e.getMessage());
        }
    }

    @Test(dataProvider = "getChangeSettingsNegativeData")
    public void changeSettingsNegativeTest(String apiName, String testCaseid, String testType, String description,
            String httpsmethod, String requestBody, String expectedResponseBody, String statusCode) throws customException {
        try {
            LogUtils.info("Starting change settings negative test case: " + testCaseid);
            ExtentReport.createTest("Change Settings Negative Test - " + testCaseid + ": " + description);
            ExtentReport.getTest().log(Status.INFO, "Test Description: " + description);
            
            if (apiName.equalsIgnoreCase("changesettings") && testType.equalsIgnoreCase("negative")) {
                requestBodyJson = new JSONObject(requestBody);
                
                LogUtils.info("Request Body: " + requestBodyJson.toString());
                ExtentReport.getTest().log(Status.INFO, "Request Body: " + requestBodyJson.toString());
                
                // Set payload for change settings request
                if (requestBodyJson.has("outlet_id")) {
                    changeSettingsRequest.setOutlet_id(requestBodyJson.getString("outlet_id"));
                }
                if (requestBodyJson.has("user_id")) {
                    changeSettingsRequest.setUser_id(requestBodyJson.getInt("user_id"));
                } else {
                    changeSettingsRequest.setUser_id(user_id);
                }
                if (requestBodyJson.has("type")) {
                    changeSettingsRequest.setType(requestBodyJson.getString("type"));
                }
                if (requestBodyJson.has("value")) {
                    changeSettingsRequest.setValue(requestBodyJson.getBoolean("value"));
                }
                ...
                // Validate status code
                    ...
                    LogUtils.success(logger, "Change settings negative test completed successfully");
                    ExtentReport.getTest().log(Status.PASS, MarkupHelper.createLabel("Change settings negative test completed successfully", ExtentColor.GREEN));
                }
            ...
            String errorMsg = "Error in change settings negative test: " + e.getMessage();
            LogUtils.exception(logger, errorMsg, e);
            ExtentReport.getTest().log(Status.FAIL, MarkupHelper.createLabel(errorMsg, ExtentColor.RED));
            if (response != null) {
                ExtentReport.getTest().log(Status.FAIL, "Failed Response Status Code: " + response.getStatusCode());
