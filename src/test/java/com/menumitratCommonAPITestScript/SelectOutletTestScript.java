...
public class SelectOutletTestScript extends APIBase
{
    ...
    @DataProvider(name = "getSelectOutletNegativeData")
    public Object[][] getSelectOutletNegativeData() throws customException {
        try {
            LogUtils.info("Reading select outlet negative test scenario data");
            ExtentReport.getTest().log(Status.INFO, "Reading select outlet negative test scenario data");
            ...
            if (filteredData.isEmpty()) {
                String errorMsg = "No valid select outlet negative test data found after filtering";
                LogUtils.failure(logger, errorMsg);
                ExtentReport.getTest().log(Status.FAIL, MarkupHelper.createLabel(errorMsg, ExtentColor.RED));
                throw new customException(errorMsg);
            }
            ...
            LogUtils.failure(logger, "Error in getting select outlet negative test data: " + e.getMessage());
            ExtentReport.getTest().log(Status.FAIL, "Error in getting select outlet negative test data: " + e.getMessage());
            throw new customException("Error in getting select outlet negative test data: " + e.getMessage());
        }
    }

    @Test(dataProvider = "getSelectOutletNegativeData")
    public void selectOutletNegativeTest(String apiName, String testCaseid, String testType, String description,
            String httpsmethod, String requestBody, String expectedResponseBody, String statusCode) throws customException {
        try {
            LogUtils.info("Starting select outlet negative test case: " + testCaseid);
            ExtentReport.createTest("Select Outlet Negative Test - " + testCaseid + ": " + description);
            ExtentReport.getTest().log(Status.INFO, "Test Description: " + description);
            
            if (apiName.equalsIgnoreCase("selectoutlet") && testType.equalsIgnoreCase("negative")) {
                requestBodyJson = new JSONObject(requestBody);
                
                LogUtils.info("Request Body: " + requestBodyJson.toString());
                ExtentReport.getTest().log(Status.INFO, "Request Body: " + requestBodyJson.toString());
                
                // Set payload for select outlet request
                ...
                // Validate status code
                    ...
                    LogUtils.success(logger, "Select outlet negative test completed successfully");
                    ExtentReport.getTest().log(Status.PASS, MarkupHelper.createLabel("Select outlet negative test completed successfully", ExtentColor.GREEN));
                }
            ...
            String errorMsg = "Error in select outlet negative test: " + e.getMessage();
            LogUtils.exception(logger, errorMsg, e);
            ExtentReport.getTest().log(Status.FAIL, MarkupHelper.createLabel(errorMsg, ExtentColor.RED));
            if (response != null) {
                ExtentReport.getTest().log(Status.FAIL, "Failed Response Status Code: " + response.getStatusCode());
