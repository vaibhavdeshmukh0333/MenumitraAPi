...
public class ChefViewTestScript extends APIBase
{
    ...
    @DataProvider(name = "getChefViewNegativeData")
    public Object[][] getChefViewNegativeData() throws customException {
        try {
            LogUtils.info("Reading chef view negative test scenario data");
            ExtentReport.getTest().log(Status.INFO, "Reading chef view negative test scenario data");
            ...
            if (filteredData.isEmpty()) {
                String errorMsg = "No valid chef view negative test data found after filtering";
                LogUtils.failure(logger, errorMsg);
                ExtentReport.getTest().log(Status.FAIL, MarkupHelper.createLabel(errorMsg, ExtentColor.RED));
                throw new customException(errorMsg);
            }
            ...
            LogUtils.failure(logger, "Error in getting chef view negative test data: " + e.getMessage());
            ExtentReport.getTest().log(Status.FAIL, "Error in getting chef view negative test data: " + e.getMessage());
            throw new customException("Error in getting chef view negative test data: " + e.getMessage());
        }
    }

    @Test(dataProvider = "getChefViewNegativeData")
    public void chefViewNegativeTest(String apiName, String testCaseid, String testType, String description,
            String httpsmethod, String requestBody, String expectedResponseBody, String statusCode) throws customException {
        try {
            LogUtils.info("Starting chef view negative test case: " + testCaseid);
            ExtentReport.createTest("Chef View Negative Test - " + testCaseid + ": " + description);
            ExtentReport.getTest().log(Status.INFO, "Test Description: " + description);
            
            if (apiName.equalsIgnoreCase("chefview") && testType.equalsIgnoreCase("negative")) {
                requestBodyJson = new JSONObject(requestBody);
                
                LogUtils.info("Request Body: " + requestBodyJson.toString());
                ExtentReport.getTest().log(Status.INFO, "Request Body: " + requestBodyJson.toString());
                
                // Set payload for chef view request
                ...
                // Validate status code
                    ...
                    LogUtils.success(logger, "Chef view negative test completed successfully");
                    ExtentReport.getTest().log(Status.PASS, MarkupHelper.createLabel("Chef view negative test completed successfully", ExtentColor.GREEN));
                }
            ...
            String errorMsg = "Error in chef view negative test: " + e.getMessage();
            LogUtils.exception(logger, errorMsg, e);
            ExtentReport.getTest().log(Status.FAIL, MarkupHelper.createLabel(errorMsg, ExtentColor.RED));
            if (response != null) {
                ExtentReport.getTest().log(Status.FAIL, "Failed Response Status Code: " + response.getStatusCode());
